object FormMain: TFormMain
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Title'
  ClientHeight = 242
  ClientWidth = 329
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 15
  object imgIcon: TImage
    Left = 11
    Top = 8
    Width = 32
    Height = 32
    AutoSize = True
    Picture.Data = {
      
      454E44AE426082}
  end
  object txtSubtitle: TLabel
    Left = 46
    Top = 8
    Width = 229
    Height = 21
    Caption = 'Dividir arquivo PDF por tamanho'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Segoe UI'
    Font.Style = []
    ParentFont = False
  end
  object Bevel1: TBevel
    Left = 46
    Top = 201
    Width = 277
    Height = 2
  end
  object btoOK: TButton
    Left = 131
    Top = 209
    Width = 92
    Height = 25
    Caption = 'btoOK'
    Default = True
    TabOrder = 0
    OnClick = btoOKClick
  end
  object btoCancel: TButton
    Left = 229
    Top = 209
    Width = 92
    Height = 25
    Cancel = True
    Caption = 'btoCancel'
    ModalResult = 2
    TabOrder = 1
  end
  object Notebook: TNotebook
    Left = 46
    Top = 45
    Width = 275
    Height = 150
    PageIndex = 1
    TabOrder = 2
    object TPage
      Left = 0
      Top = 0
      Caption = 'Start'
      object txtTypeMaxSize: TLabel
        Left = 0
        Top = 0
        Width = 273
        Height = 15
        AutoSize = False
        Caption = 'txtTypeMaxSize'
      end
      object editSize: TEdit
        Left = 0
        Top = 23
        Width = 194
        Height = 23
        NumbersOnly = True
        TabOrder = 0
      end
      object comboSizeType: TComboBox
        Left = 200
        Top = 23
        Width = 74
        Height = 23
        Style = csDropDownList
        ItemIndex = 1
        TabOrder = 1
        Text = 'MB'
        Items.Strings = (
          'KB'
          'MB')
      end
      object checSave: TCheckBox
        Left = 0
        Top = 61
        Width = 272
        Height = 17
        Caption = 'checSave'
        Checked = True
        State = cbChecked
        TabOrder = 2
      end
      object checReplace: TCheckBox
        Left = 0
        Top = 84
        Width = 272
        Height = 34
        Caption = 'checReplace'
        Checked = True
        State = cbChecked
        TabOrder = 3
        WordWrap = True
      end
      object checDelete: TCheckBox
        Left = 0
        Top = 124
        Width = 272
        Height = 17
        Caption = 'checDelete'
        Checked = True
        State = cbChecked
        TabOrder = 4
      end
    end
    object TPage
      Left = 0
      Top = 0
      Caption = 'Progress'
      object txtDescStatus: TLabel
        Left = 0
        Top = 0
        Width = 70
        Height = 15
        Caption = 'txtDescStatus'
      end
      object txtStatus: TLabel
        Left = 227
        Top = 44
        Width = 45
        Height = 15
        Alignment = taRightJustify
        Caption = 'txtStatus'
      end
      object barProcess: TProgressBar
        Left = 0
        Top = 21
        Width = 273
        Height = 17
        TabOrder = 0
      end
    end
  end
end
